...
$category = $db->fetchsql("SELECT * FROM category WHERE level=3 AND home=1");

if($the == "")
{
    $sqlRated = "SELECT product.* FROM product LEFT JOIN category ON category.id = product.category_id WHERE category.level = 3 ORDER BY rated DESC";

}
else
{
    $sqlRated = "SELECT product.* FROM product LEFT JOIN category ON category.id = product.category_id WHERE category.level = 3 AND product.slug LIKE'%".$the."%' ";
}
...
<div class="tabslink">
    <a href="maytinhdeban.php" class>Máy tính bộ - Màn hình</a>
    <a href="mayin.php" class="mayin">Máy in</a>
</div>
    ...
    <div id="owl-home" class="owl-carsourel owl-theme" style="opacity: 1; display:block;">
        <div class="owl-wrapper-outer">
            <div class="owl-wrapper" style="width: 20800px;">
                <div class="owl-item">
                    <img src="images/banner-pc/mayin1.jpg" style="width: 765px">
                </div>
            </div>
        </div>
